...
import * as globalProcessHandler from '../process/global.js'
import * as juejinProcessHandler from '../process/juejin.im.js'
...
`
  ...
  while (pinCursor < pins.length) {
    ...
    for (const pin of chunkedPins) {
      ...
      pinsRendered.push(
        _.template(template)({
          ...
          date: day(pin.msg_Info.ctime * 1000).format('YYYY-MM-DD HH:mm'),
    ...
    if (opts.less) {
      ...
      await consoleReader(
        await marked.parse(pinsRendered.join('\n\n---\n\n')),
        {
      ...
      pinCursor++
      goon = 'next'
    } else if (opts.mdcat && shell.which('mdcat')) {
      clearConsole()
      pinsRendered.push(
        '按空格或回车下一条，按 q 或 CTRL+c 退出，按 h 查看帮助'
      )

      const tmpPath = (await consoleReader(pinsRendered.join('\n\n---\n\n'), {
        plugin: 'semo-plugin-juejin',
        identifier: topic_id + '',
        tmpPathOnly: true,
      })) as string
      ...
      try {
        await execPromise(`mdcat ${tmpPath}`, undefined, () => {
          unlinkSync(tmpPath)
        })
      } catch {}
...
| CTRL+c/q | 退出 |
        ...
        `)
        console.log(await marked.parse(helpLines.join('\n\n---\n\n')))
      ...
      console.log(await marked.parse(pinsRendered.join('\n\n---\n\n')))
  ...
  const selectTopic = await inquirer.prompt([
    {
      ...
      source: (answers, input) => {
        input = input || ''
        let i = 0
        const fuzzyResult = fuzzy.filter(input, topics, {
          extract: (el: any) => {
...
export async function posts(
  ...
  let posts,
    cursor = '0'
  const data = await getPosts(cate_id, tag_id, sort_type, cursor, opts.uuid)
  posts = data.posts
  cursor = data.cursor
...
`)
    ...
    if (!opts.copyOnly) {
      if (opts.mdcat && shell.which('mdcat')) {
        clearConsole()

        const tmpPath = (await consoleReader(
        ...
        try {
          await consoleReader(await marked.parse(template), {
            plugin: 'semo-plugin-juejin',
            identifier: post_id,
          })
        } catch {}
        ...
        console.log(await marked.parse(postList.join('\n\n---\n\n')))
      ...
      success(`Copied to system clipboard successfully.`)
      return false
    }
  }
}
  ...
  const selectSort = await inquirer.prompt([
    {
      ...
      source: (answers, input) => {
        input = input || ''
        let i = 0
        const fuzzyResult = fuzzy.filter(input, sorts, {
          extract: (el: any) => {
